...
/** Speckle: a field of grey where every level of a band appears as often as every other.
  ...
  *
  * Static rather than re-drawn each frame. A field that scintillated would inject broadband noise of its own into every
  * quadrant, which is a different experiment: here the texture belongs to the scene, and only the figure moves.
  */
private[fe] object Texture:
  ...
  /** Every level of the band exactly as often as every other, in scrambled order.
    ...
    * visibly ragged -- some levels a few percent over, others under -- and the whole point of naming a band is that the
    * test covers it evenly, not on average over enough runs.
    */
