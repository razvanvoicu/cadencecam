...
private[fe] object SignalGraph:
  ...
  /** The value range a pane draws against, for the chosen zoom.
    *
    * Every step is a fixed, stated span, so a given change in brightness always draws the same height at a given
    * zoom and the panes stay comparable with each other. What changes with the data is only where that span sits:
    * it is centred on the window's own mean, so slow drift does not push the trace off the pane. The span never
    * widens to fit the data, which is what separates this from scaling each window to its own extent.
    *
  ...
  /** Horizontal position of a sample, in the range 0 to 1.
    *
    * The window is always [[WindowSamples]] wide, so while the buffer is still filling the trace grows rightwards
    * from the left edge, and only once full does it start sliding — which is what makes the motion readable rather
    * than a line that rescales under you.
    */
  ...
  /** One line to draw. Each trace is centred on its own mean, so series with different resting levels can share a
    * pane and be compared by shape.
    */
  ...
  def draw(canvas: dom.HTMLCanvasElement, traces: Seq[Trace], zoom: SignalZoom, grid: String): Unit =
    ...
    traces.filter(_.samples.nonEmpty).foreach: trace =>
...
/** How much of the brightness scale a trace shows, as a half-width about the series' own mean.
  ...
  * which needs the height of a step to mean the same thing from one moment to the next. ±128 spans the whole
  * 0-255 range, so it is the widest view worth having.
  */
