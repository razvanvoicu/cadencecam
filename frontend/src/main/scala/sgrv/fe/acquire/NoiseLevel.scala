...
/** How much the picture moves when nothing in it does.
  *
  * Reported because it decides whether counting is possible at all, and because the remedy is something only the
  * person holding the camera can apply: move back, add light, put the phone on something solid. A count that drifts
  * upward while a room is empty looks like a fault in the app, and on a noisy camera it is not one.
  *
...
private[fe] object NoiseLevel:
  ...
  /** Where noise stops being harmless and starts setting the bar itself.
    ...
    * routinely exceed one and a half times their own RMS. That is the point at which a camera becomes unusable, and
    * it is worth telling someone before they run a set rather than after.
    */
