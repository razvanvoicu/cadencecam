...
private[be] object RouteDiscovery:
  ...
  private def scan(classLoader: ClassLoader): Task[Seq[String]] =
    ModuleDiscovery.implementations(classOf[BackendPlugin], classLoader)

  private[be] def load(className: String, classLoader: ClassLoader): Task[BackendPlugin] =
    ModuleDiscovery.load(className, classOf[BackendPlugin], classLoader)
  ...
  /** The route discovery process returns a list of statuses. The actual routes are buried inside the `Active` status
    * objects, and they need to be dug out. Non-active statuses are silently ignored.
    */
