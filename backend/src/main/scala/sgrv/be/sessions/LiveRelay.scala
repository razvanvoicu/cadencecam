...
/** Carries readings from an account's acquirer to its dashboards, and commands back the other way.
  *
  * Both ends authenticate as themselves and are paired by account, so nothing in either message says whose count it is:
  * a device can report or command only for the user holding it. The dashboard cannot address an acquirer at all, only
...
object LiveRelay extends BackendPlugin:
  ...
  /** The event that means a server-side socket is ready to use.
    *
    * Not `Registered`, which is Netty's channel-registration callback and fires when a connection is first accepted. On
    * the server the WebSocket handler joins the pipeline only after the upgrade, by which time registration is long
    * past, so `Registered` never arrives and anything waiting for it waits forever. The handshake completing is the
    * signal that actually marks a usable socket at this end.
    */
  ...
  /** Distinguishes one connection from another for the same account, so a socket that goes away removes itself rather
    * than whichever one happens to be registered.
    */
